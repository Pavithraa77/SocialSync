...
def sign_up():
    ...
    interest = input("Enter your interest: ").strip()  # Add interest input

    if main_server.get_vertex(userid) is None:
        main_server.insertVertex(userid, password, name, age, gender,
                                 phone, email, location, interest)  # Include interest
        print("\n" + "="*40)
        print(" Sign Up Successful! You can now log in. ".center(40, "="))
        print("="*40)

        # Append the new user details to user.csv
        with open('user.csv', 'a', newline='') as csvfile:
            fieldnames = ['User ID', 'Password', 'Name', 'Age', 'Gender',
                          'Phone', 'Email', 'Location', 'Interest']  # Include Interest
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            # Write the new user data
            writer.writerow({'User ID': userid, 'Password': password, 'Name': name, 'Age': age,
                             'Gender': gender, 'Phone': phone, 'Email': email, 'Location': location,
                             'Interest': interest})  # Include Interest
    else:
        print("\n" + "="*40)
        print(" User ID already exists. Please choose a different ID. ".center(40, "="))
        print("="*40)
            ...
            if check is None:
                ...
                while True:
                    ...
                    if choice3 == 1:
                        ...
                        u = input(
                            "Enter userid from table you want to follow : ")
                        print()
                        ...
                        range = int(
                            input("Enter the range to see friends (in km): "))
                        starting_city = check.location
                        max_distance = range  # Maximum distance to consider

                        starting_vertex = location_server.get_vertex(
                            starting_city)
                        if starting_vertex:
                            surrounding_cities = bfs(
                                south_india_graph, starting_vertex, max_distance)
                            print("\n" + "="*40)
                            print(
                                f" Cities within {max_distance} km from {starting_city} ".center(40, "="))
                            print("="*40)
                            print(surrounding_cities)
                            find_city = location_server.get_vertex(
                                starting_city)
                        ...
                        if user is None:
                            ...
                            choice4 = int(
                                input("Enter 1 if you want to follow : "))
                            if choice4 == 1:
                        ...
                        print("\n" + "="*40)
                        print(
                            " Invalid choice. Please enter a number between 1 and 7. ".center(40, "="))
                        print("="*40)
                        continue
